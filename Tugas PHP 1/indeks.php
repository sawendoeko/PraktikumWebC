...
function tambah($koneksi){
	...
	if (isset($_POST['btn_simpan'])){
		...
		if(!empty($namalengkap) && !empty($namauser) && !empty($nim) && !empty($kelamin) && !empty($tempat) && !empty($tanggal) && !empty($asal) && !empty($jurusan) && !empty($pass)){
			...
			$sql = "INSERT INTO praktikum (id, namalengkap, namauser, nim, kelamin, tempat, tanggal, asal, jurusan, pass) VALUES ('$id', '$namalengkap', '$namauser', '$nim', '$kelamin', '$tempat', '$tanggal', '$asal', '$jurusan', '$pass')";
...
function ubah($koneksi){
	...
	if(isset($_POST['btnubah'])){
		...
		if(!empty($id) &&!empty($namalengkap) && !empty($namauser) && !empty($nim) && !empty($kelamin) && !empty($tempat) && !empty($tanggal) && !empty($asal) && !empty($jurusan) && !empty($pass)){
			...
			$sql_update = "UPDATE `praktikum` SET `namalengkap`='$namalengkap',`namauser`='$namauser',`nim`='$nim',`kelamin`='$kelamin' ,`tempat`='$tempat',`tanggal`='$tanggal',`asal`='$asal',`jurusan`='$jurusan',`pass`='$pass' WHERE `id`='$id'";
...
function hapus($koneksi){
	...
	if(isset($_GET['id']) && isset($_GET['aksi'])){
		...
		$sql_hapus = "DELETE FROM praktikum WHERE id=" . $id;
